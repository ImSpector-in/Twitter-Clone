...
import { getProfileByUsername, getFollowing, getRelationshipStatus } from '@/lib/queries/profile'
...
export default async function FollowingPage({ params }: Props) {
  ...
  const [following, myFollowsResult] = await Promise.all([
  ...
  return (
    <div>
      ...
      {following.length === 0 ? (
        ...
        <ul>
          {(following as unknown as Array<{ id: string; username: string; display_name: string | null; avatar_url: string | null }>).map((f) => {
